    ...
    public class DataRepository
    {
        ...
        public ICollection<RefuelViewModel> GetRefuelList(string sDate)
        {
            using (var _db = new LocalDbContext())
            {
                ...
                return list.OrderBy(r => r.FlightCode).ToList();
            }
        ...
        public RefuelViewModel GetRefuel(int id)
        {
            ...
            if (UseRemote())
            {
                ...
                {
                    if (localItem == null)
                        localItem = new LocalRefuel { Id = remoteItem.Id, Date = remoteItem.RefuelTime.Value.ToString("yyyyMMdd"), JsonData = JsonConvert.SerializeObject(remoteItem), Synced = true };
                    else if (localItem.Synced)
                    {
        ...
        public RefuelViewModel PostRefuel(RefuelViewModel model)
        {
            ...
            if (!canPost)
            {
                ...
                {
                    localItem = _db.Refuels.FirstOrDefault(lr => lr.Id == model.Id || lr.Key == model.LocalGuid);
                    if (localItem != null)
                    {
        ...
        {
            var localItem = _db.Invoices.FirstOrDefault(lr => lr.Id == id);
            if (UseRemote())
            {
                ApiHelper client = new ApiHelper();
                ...
                {
                    if (localItem == null)
                        localItem = new LocalInvoice { Id = remoteItem.Id, InvoiceNumber = remoteItem.InvoiceNumber, JsonData = JsonConvert.SerializeObject(remoteItem), Synced = true };
                    else if (localItem.Synced)
                    {
                        localItem.JsonData = JsonConvert.SerializeObject(remoteItem);
                    }
